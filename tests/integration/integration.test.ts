 ...
 *
 * These tests verify the end-to-end flow of the plugin.
...
describe.skipIf(skipIntegration)("Integration Tests", () => {
  ...
  describe("Message Sending", () => {
    ...
    it("should send markdown message to user", async () => {
      ...
      const result = await sendToUser(config, testUserId, "# Integration Test\n\nThis is a **markdown** message.", { useAICard: false });
    ...
    it("should send AI card to user", async () => {
      ...
      const result = await sendToUser(config, testUserId, "AI Card content for integration test", { useAICard: true });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle rate limiting", async () => {
      ...
      const promises = [];
      for (let i = 0; i < 5; i++) {
        promises.push(sendToUser(config, testUserId, `Rate limit test ${i}`, { useAICard: false }));
      }
