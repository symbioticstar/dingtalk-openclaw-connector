...
describe("resolveAgentIdByBindings", () => {
  ...
  it("priority 1: exact peer.kind + peer.id match", () => {
    ...
    mockReadFileSync.mockReturnValue(
      JSON.stringify({
        bindings: [{ agentId: "agent1", match: { channel: "dingtalk-connector", peer: { kind: "direct", id: "user1" } } }],
      }),
    );
    const out = resolveAgentIdByBindings("acc1", "direct", "user1", log);
  ...
  it("priority 1: no match when peerId differs", () => {
    ...
    mockReadFileSync.mockReturnValue(
      JSON.stringify({
        bindings: [{ agentId: "agent1", match: { peer: { kind: "direct", id: "user1" } } }],
      }),
    );
  ...
  it("priority 1: accountId filter - skip when accountId does not match", () => {
    ...
    mockReadFileSync.mockReturnValue(
      JSON.stringify({
        bindings: [{ agentId: "a", match: { accountId: "acc1", peer: { kind: "direct", id: "u1" } } }],
      }),
    );
