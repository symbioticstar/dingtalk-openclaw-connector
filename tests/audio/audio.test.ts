...
// Mock child_process for extractAudioDuration (promisified)
const mockExecFileAsync = vi.hoisted(() => vi.fn());
const mockExecFile = vi.hoisted(() => {
  const fn = vi.fn();
  (fn as any)[Symbol.for("nodejs.util.promisify.custom")] = mockExecFileAsync;
  return fn;
});
...
describe("audio helpers", () => {
  ...
  describe("getFfprobePath", () => {
    it("should return FFPROBE_PATH env variable when package not found", async () => {
      ...
      const path = await getFfprobePath();
  ...
  describe("extractAudioDuration", () => {
    it("should return duration in milliseconds on success", async () => {
      ...
      mockExecFileAsync.mockResolvedValue({
        stdout: JSON.stringify({ format: { duration: "123.45" } }),
        stderr: "",
      });
    ...
    it("should return null when execFile fails", async () => {
      ...
      mockExecFileAsync.mockRejectedValue(new Error("ffprobe failed"));
