...
describe("upload functionality", () => {
  ...
  describe("processFileMarkers", () => {
    it("should process file markers", async () => {
      ...
      expect(mockAxiosPost.mock.calls.some((c) => String(c[0]).includes("&type=file"))).toBe(true);
    });
  });
  ...
  describe("processAudioMarkers", () => {
    it("should process audio markers", async () => {
      ...
      expect(mockAxiosPost.mock.calls.some((c) => String(c[0]).includes("&type=voice"))).toBe(true);
    });
  });
});
